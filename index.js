...
controller.hears(
  ...
  (bot, message) => {
    ...
    getBalance(target, balance => {
      var reply =
        user == target
          ? `You have ${balance}gp in your account, hackerlacker.`
...
var invoice = async (
  ...
  bot.say({
    user: '@' + recipient,
    channel: '@' + recipient,
    text: `Good morrow hackerlacker. <@${sender}> has just sent you an invoice of ${amount}gp${replyNote}
      ...
      getBalance(target, (targetBalance, targetRecord) => {
        ...
        if (channelType == 'im') {
          bot.say({
            user: '@' + target,
            channel: '@' + target,
            text: `Good morrow hackerlacker. <@${user}> has just transferred ${amount}gp to your account${replyNote}`
          });
    ...
    if (command == '/balance') {
      ...
      if (match) {
        ...
        getBalance(target, balance => {
          var reply =
            user == target
              ? `Ah yes, <@${target}> (${target}). You have ${balance}gp in your account, hackerlacker.`
