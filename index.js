...
var invoice = async (
    ...
    bot.say({
       ...
       Reply with "@banker pay ${invRecord.id}".`.replace("@channel", "").replace("@everyone", "")
    });
};
            ...
            getBalance(target, (targetBalance, targetRecord) => {
                ...
                if (channelType == 'im') {
                    bot.say({
                        user: '@' + target,
                        channel: '@' + target,
                        text: `Good morrow hackalacker. <@${user}> has just transferred ${amount}gp to your account${replyNote}`.replace("@channel", "").replace("@everyone", "")
                    });
                    ...
                    bot.say({
                        user: '@' + target,
                        channel: '@' + target,
                        text: `$$$ | <@${user}> | ${amount} | ${replyNote} | ${channelid} | ${ts}`.replace("@channel", "").replace("@everyone", "")
                    });
                }
...
controller.hears(
    ...
    (bot, message) => {
        ...
        var replyCallback = text => bot.replyInThread(message, text.replace("@channel", "").replace("@everyone", ""));
...
controller.hears(
    ...
    async (bot, message) => {
        ...
        var replyCallback = (text, wentThrough) => {
            bot.replyInThread(message, text.replace("@channel", "").replace("@everyone", ""));
            if (typeof invoiceReplies[id] == 'function' && wentThrough) {
                invoiceReplies[id](
                    `<@${user}> paid their invoice of ${amount} gp from <@${target}>${invRecord.fields['Reason']}`.replace("@channel", "").replace("@everyone", "")
                );
            }
        };
                        ...
                        blocks: [
                            ...
                            },
                            {
                                ...
                                elements: [
                                    {
                                        type: 'mrkdwn',
                                        text: `Transferred by <@${user_id}>`.replace("@channel", "").replace("@everyone", "")
                                    }
                                ]
                            }
                        ]
